...
public class StatsService : IStatsService<ulong>
{
	...
	private readonly IPlayerStatsRepository<
		PlayerStat,
		uint
	> playerStatRepository;
	private readonly IClassStatsRepository<
		ClassStat,
		uint
	...
	public IQueryable<JoinedStats> PlayerStatsJoinQueryable()
	{
		...
		var InnerJoinQuery =
			...
			join playerStat in playerStats
				on player.SteamId equals playerStat.SteamId
			join classStat in classStats
				on playerStat.PlayerStatsId equals classStat.PlayerStatsId
			join gameStat in gameStats
				on playerStat.GameId equals gameStat.GameId
	...
	{
		var playerNumberOfGames = PlayerGameCounter(
			StatsCollectionConstants.ALLTIME_THRESHOLD
		);
		var playerGeneralStats = MainStatsCollector(
			StatsCollectionConstants.ALLTIME_THRESHOLD
		);
		var playerAirshots = AirshotStatsCollector(
			StatsCollectionConstants.ALLTIME_THRESHOLD
		);
		var playerHeadshots = HeadshotStatsCollector(
			StatsCollectionConstants.ALLTIME_THRESHOLD
		);
		...
		var completeLeaderboard = (
			...
			where
				png.NumberOfGames
				> StatsCollectionConstants.PLAYER_NUMBEROFGAMES_THRESHOLD
	...
	private List<PlayerNumGames> PlayerGameCounter(
		int timeFrame,
		int classID = 0,
		ulong steamID = 0
	)
	{
		...
		groupedPlayerGames = groupedPlayerGames.Where(
			player => player.FirstOrDefault()?.PlayerName != null
		);

		var groupedPlayerDistinctGames = groupedPlayerGames.Select(
			game =>
				new PlayerNumGames(
					game.Key,
					game.FirstOrDefault()?.PlayerName is not null
				...
				new AverageMainStats(
					...
					(
						avg
							...
							: player.Max(k => k.Kills)
					),
					(
						avg
							? player.Average(a => a.Assists)
							: player.Max(a => a.Assists)
					),
					(
						avg
							? player.Average(de => de.Deaths)
							: player.Max(de => de.Deaths)
					)
				)
		);
	...
	private List<AverageAirshots> AirshotStatsCollector(
		int timeFrame,
		int classID = 0,
		ulong steamID = 0,
		bool avg = true
	)
	{
		...
		var averagedAirshots = groupedPlayerGames.Select(
			player =>
				new AverageAirshots(
					player.Key,
					(
	...
	private List<AverageMedicStats> MedicStatsCollector(
		int timeFrame,
		ulong steamID = 0,
		bool avg = true
	)
	{
		...
		var averagedMedicStats = groupedPlayerGames.Select(
			player =>
				new AverageMedicStats(
					...
					(
						avg
							...
							: player.Max(d => d.Drops)
					),
					(
						avg
							? player.Average(u => u.Ubers)
							: player.Max(u => u.Ubers)
					),
					(
						avg
							? player.Average(
	...
	private List<AverageHeadshots> HeadshotStatsCollector(
		int timeFrame,
		ulong steamID = 0,
		bool avg = true
	)
	{
		...
		var averagedHeadshots = groupedPlayerGames.Select(
			player =>
	...
	{
		var playerNumberOfGames = PlayerGameCounter(
			StatsCollectionConstants.RECENTGAMES_THRESHOLD
		);
		var playerGeneralStats = MainStatsCollector(
			StatsCollectionConstants.RECENTGAMES_THRESHOLD
		);
	...
	{
		var scoutGames = PlayerGameCounter(
			StatsCollectionConstants.ALLTIME_THRESHOLD,
			StatsCollectionConstants.SCOUT_CLASSID
		);
		var scoutPerformance = MainStatsCollector(
			StatsCollectionConstants.ALLTIME_THRESHOLD,
			StatsCollectionConstants.SCOUT_CLASSID
		);
	...
	{
		var soldierGames = PlayerGameCounter(
			StatsCollectionConstants.ALLTIME_THRESHOLD,
			StatsCollectionConstants.SOLDIER_CLASSID
		);
		var SoldierPerformance = MainStatsCollector(
			StatsCollectionConstants.ALLTIME_THRESHOLD,
			StatsCollectionConstants.SOLDIER_CLASSID
		);
		var soldierAirshots = AirshotStatsCollector(
			StatsCollectionConstants.ALLTIME_THRESHOLD,
			StatsCollectionConstants.SOLDIER_CLASSID
		);
		...
		if (steamid != 0)
		{
			completeLeaderboard = completeLeaderboard.Where(
	...
	{
		var demomanGames = PlayerGameCounter(
			StatsCollectionConstants.ALLTIME_THRESHOLD,
			StatsCollectionConstants.DEMOMAN_CLASSID
		);
		var demomanPerformance = MainStatsCollector(
			StatsCollectionConstants.ALLTIME_THRESHOLD,
			StatsCollectionConstants.DEMOMAN_CLASSID
		);
		var demomanAirshots = AirshotStatsCollector(
			StatsCollectionConstants.ALLTIME_THRESHOLD,
			StatsCollectionConstants.DEMOMAN_CLASSID
		);
	...
	{
		var medicGames = PlayerGameCounter(
			StatsCollectionConstants.ALLTIME_THRESHOLD,
			StatsCollectionConstants.MEDIC_CLASSID
		);
		var medicCombatStats = MainStatsCollector(
			StatsCollectionConstants.ALLTIME_THRESHOLD,
			StatsCollectionConstants.MEDIC_CLASSID
		);
		var medicHealingStats = MedicStatsCollector(
			StatsCollectionConstants.ALLTIME_THRESHOLD
		);
	...
	{
		var scoutHighlights = MainStatsCollector(
			StatsCollectionConstants.ALLTIME_THRESHOLD,
			StatsCollectionConstants.SCOUT_CLASSID,
			steamID,
			false
		);
		var soldierHighlights = MainStatsCollector(
			StatsCollectionConstants.ALLTIME_THRESHOLD,
			StatsCollectionConstants.SOLDIER_CLASSID,
			steamID,
			false
		);
		var maxSoldierAirshots = AirshotStatsCollector(
			StatsCollectionConstants.ALLTIME_THRESHOLD,
			StatsCollectionConstants.SOLDIER_CLASSID,
			steamID,
			false
		);
		var demomanHighlights = MainStatsCollector(
			StatsCollectionConstants.ALLTIME_THRESHOLD,
			StatsCollectionConstants.DEMOMAN_CLASSID,
			steamID,
			false
		);
		var maxDemomanAirshots = AirshotStatsCollector(
			StatsCollectionConstants.ALLTIME_THRESHOLD,
			StatsCollectionConstants.DEMOMAN_CLASSID,
			steamID,
			false
		);
		var medicHighlights = MedicStatsCollector(
			StatsCollectionConstants.ALLTIME_THRESHOLD,
			steamID,
			false
		);
		...
		var playerHighlights = (
			...
			join soldierGame in soldierHighlights
				on scoutGame.SteamId equals soldierGame.SteamId
			join sAirshots in maxSoldierAirshots
				on scoutGame.SteamId equals sAirshots.SteamId
			join demoGame in demomanHighlights
				on scoutGame.SteamId equals demoGame.SteamId
			join dAirshots in maxDemomanAirshots
				on scoutGame.SteamId equals dAirshots.SteamId
			join medicGame in medicHighlights
				on scoutGame.SteamId equals medicGame.SteamId
